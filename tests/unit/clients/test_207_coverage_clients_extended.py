...
class TestMeilisearchClientCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("ffbb_api_client_v3.clients.meilisearch_client.http_post_json_async", new_callable=AsyncMock)
...
class TestApiFFBBAppClientCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    def test_get_configuration(self, mock_get):
        mock_get.return_value = {"data": {"id": "1", "key_dh": "a", "key_ms": "b", "maintenance": False}}
    ...
    @patch("ffbb_api_client_v3.clients.api_ffbb_app_client.http_get_json_async", new_callable=AsyncMock)
    async def test_get_configuration_async(self, mock_get_async):
        mock_get_async.return_value = {"data": {"id": "1", "key_dh": "a", "key_ms": "b", "maintenance": False}}
        client = ApiFFBBAppClient("token123")
    ...
    def test_list_competitions_none_fields(self):
        ...
        with patch("ffbb_api_client_v3.clients.api_ffbb_app_client.http_get_json") as mock_get:
...
class TestFFBBAPIClientV3Coverage(unittest.IsolatedAsyncioTestCase):
    def test_create(self):
        client = FFBBAPIClientV3.create("meili_token_with_enough_length", "api_token_with_enough_length")
    ...
    def test_create_with_sessions(self):
        ...
        client = FFBBAPIClientV3.create("meili_token_with_enough_length", "api_token_with_enough_length", cached_session=session, async_cached_session=async_session)
    ...
    async def test_async_delegations(self):
        ...
        meili_client.recursive_smart_multi_search_async.return_value = MagicMock(results=["res"])
        ...
        self.assertEqual(await client.search_multiple_competitions_async(["A"]), ["res"])
        ...
        # rencontres
        from ffbb_api_client_v3.models.multi_search_result_rencontres import RencontresMultiSearchResult
        ...
        from ffbb_api_client_v3.models.competition_id_categorie import CompetitionIDCategorie
        
        hit1 = RencontresHit(id="1", competition_id=CompetitionID(id="1", categorie=CompetitionIDCategorie(code="U15", libelle="U15")))
        hit2 = RencontresHit(id="2", competition_id=CompetitionID(id="2", categorie=CompetitionIDCategorie(code="U18", libelle="U18")))
        
        res_rencontres = RencontresMultiSearchResult(hits=[hit1, hit2], query="", processing_time_ms=0, limit=10, offset=0, estimated_total_hits=2)
        meili_client.recursive_smart_multi_search_async.return_value = MagicMock(results=[res_rencontres])
        ...
        meili_client.recursive_smart_multi_search_async.return_value = MagicMock(results=[])
        ...
        res_rencontres2 = RencontresMultiSearchResult(hits=[hit1, hit2], query="", processing_time_ms=0, limit=10, offset=0, estimated_total_hits=2)
        meili_client.recursive_smart_multi_search.return_value = MagicMock(results=[res_rencontres2])
        ...
        meili_client.recursive_smart_multi_search_async.return_value = MagicMock(results=["res"])
        meili_client.recursive_smart_multi_search = MagicMock()
        meili_client.recursive_smart_multi_search.return_value = MagicMock(results=["res"])
